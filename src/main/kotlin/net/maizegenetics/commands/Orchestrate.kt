...
class Orchestrate : CliktCommand(name = "orchestrate") {
    ...
    override fun run() {
        ...
        when (config.version) {
            "v2" -> runV2Pipeline(config, workDir)
            else -> runV1Pipeline(config, workDir)
        }
    }
     ...
     */
    private fun runV1Pipeline(config: PipelineConfig, workDir: Path) {
        logger.info("=".repeat(80))
     ...
     * intentionally kept independent from [runV1Pipeline] so it can diverge
     ...
     */
    private fun runV2Pipeline(config: PipelineConfig, workDir: Path) {
        logger.info("=".repeat(80))
        logger.info("Starting Pipeline Orchestration")
