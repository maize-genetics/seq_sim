...
Sequence simulator pipeline and orchestrator for [GRITS](https://github.com/maize-genetics/grits). The primary purpose of this project is to be a comprehensive bioinformatics pipeline for assembly alignment, variant simulation, and comparative analysis.
...
```bash
# 0. Install seq_sim (see Installation above) and make sure it's on your PATH.
...
See [`docs/commands.md`](docs/commands.md) for the full reference covering every
command's options, behavior, outputs, and examples — including `orchestrate`,
the 15 pipeline steps, and the helper commands (`extract-chrom-ids`,
`mutate-assemblies`, `recombine-gvcfs`).
...
# 3. Run steps 01-04 (see each command's section above for more examples)
...
> pipeline step numbers in the Commands and Pipeline Overview sections are the
> canonical ordering.
